...
const RegisterForm = ({ user }) => {
    ...
    const [initApply, setInitApply] = useState(false);
    const [apply, setApply] = useState(INIT_APPLY);
    const [disabled, setDisabled] = useState(true); // Define the 'disabled' state
    ...
    <div className="form-group">
        <textarea
            name="as_teacher_req_desc"  // Corrected name attribute
            cols="30"
            rows="5"
            placeholder="Please tell us about your teaching profession"
            className={`form-control ${errors.as_teacher_req_desc ? 'is-invalid' : ''}`}
            value={apply.as_teacher_req_desc}
            onChange={handleChange}
            ref={register({ required: true })}
        />
        {errors.as_teacher_req_desc && (
